...
// Q32
const currentusers=['Admin', 'Newton', 'Sikander', 'Raees', 'ALi', 'Mubashir']
const newusers=['Admin', 'Newton', 'Sikander', 'Usama', 'Mubashir', 'Tariq']

function checkUserName(currentusers){
    const lowercaseCurrentUsers=currentusers.map(user => user.toLowerCase())

    for(const newUser of newusers){
        const lowercaseNewUser=newUser.toLocaleLowerCase()
